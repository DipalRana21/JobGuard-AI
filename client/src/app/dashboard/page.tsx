...
import {
  ...
  Search, Activity, AlertTriangle, Plus, History
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  const criticalThreatsCount = scans.filter(scan => scan.riskScore >= 70).length;

  const filteredScans = scans.filter(scan => 
    scan.companyName.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen bg-[#0b1220] text-white flex font-sans relative overflow-hidden">
      ...
      {/* --- SIDEBAR --- */}
      <aside className="w-72 border-r border-white/10 bg-gradient-to-b from-[#111827]/90 to-[#0f172a]/90 backdrop-blur-2xl p-8 flex flex-col justify-between hidden md:flex shadow-xl z-10">
        <div>
          ...
          <nav className="space-y-3">
            ...
            {/* 👇 Removed redundant New Scan button from here. Much cleaner! */}

            <button
              onClick={handleLogout}
              className="w-full flex items-center gap-3 px-5 py-3 text-red-400 hover:bg-red-500/10 rounded-xl font-medium transition-all mt-8"
            >
      ...
      <main className="flex-1 p-8 md:p-12 overflow-y-auto relative z-10">
        ...
        <header className="flex justify-between items-center mb-12">
          <div className="relative w-full max-w-md hidden md:block">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-full py-2.5 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-colors placeholder:text-gray-600"
            />
          </div>
        ...
        {/* 👇 Wipes the welcome text when searching */}
        {!searchQuery && (
          <div className="mb-10">
        ...
        {/* 👇 CONDITIONAL RENDERING: This entirely hides the 3 stat grids if there is ANY text in the search bar */}
        {!searchQuery && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-16">
            ...
            <Link href="/" className="relative group h-full">
              ...
              <div className="relative bg-gradient-to-br from-blue-600 to-purple-700 p-10 rounded-3xl border border-white/10 transition-all duration-300 shadow-[0_0_50px_rgba(99,102,241,0.25)] hover:-translate-y-1 h-full flex flex-col items-center justify-center text-center">
                ...
                <p className="text-base text-blue-200">Initialize Investigation</p>
              </div>
            </Link>
        ...
        <div>
          {/* 👇 Dynamic title changes from "Recent Reports" to "Search Results" automatically */}
          <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
            {searchQuery ? <Search className="w-5 h-5 text-blue-400" /> : <History className="w-5 h-5 text-gray-400" />} 
            {searchQuery ? `Search Results for "${searchQuery}"` : "Recent Intelligence Reports"}
          </h2>
          ...
          {loadingScans ? (
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-12 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-lg font-bold text-white mb-2">
                {searchQuery ? "No matching reports found" : "No scans recorded yet"}
              </h3>
              <p className="text-gray-400 max-w-sm mb-6 text-sm">
                {searchQuery ? "Try adjusting your search terms." : "Start a new scan to analyze a company's digital footprint."}
              </p>
              {!searchQuery && (
              ...
              {filteredScans.map((scan) => (
                ...
                >
                  <div className="bg-[#111827]/95 p-8 rounded-2xl border border-white/10 hover:border-blue-500/40 transition-all shadow-lg hover:-translate-y-1 overflow-hidden">
                    ...
                    <div className={`absolute -top-10 -right-10 w-40 h-40 blur-3xl opacity-20 ${
                        scan.riskScore < 30 ? "bg-green-500" :
                    ...
                    <div className="flex justify-between mb-6 text-sm">
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider ${
                          scan.riskScore < 30 ? "bg-green-500/10 text-green-400" :
                          scan.riskScore < 70 ? "bg-yellow-500/10 text-yellow-400" :
                          "bg-red-500/10 text-red-400"
                        }`}>
                        {scan.status || (scan.riskScore >= 70 ? "Critical" : scan.riskScore >= 30 ? "Suspicious" : "Safe")}
                      </span>
                    </div>
